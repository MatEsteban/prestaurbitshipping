...
if (!class_exists('LicenceApi')) {
    require_once(dirname(__FILE__) . '/../classes/urbit/LicenceApi.php');
}
if (!class_exists('UrbitApi')) {
    require_once(dirname(__FILE__) . '/../classes/urbit/UrbitApi.php');
}
if (!class_exists('UrbitPackage')) {
    require_once(dirname(__FILE__) . '/../models/UrbitPackage.php');
}
if (!class_exists('UrbitExtraCover')) {
    require_once(dirname(__FILE__) . '/../models/UrbitExtraCover.php');
}
...
if (!class_exists('UrbitCache')) {
    require_once(dirname(__FILE__) . '/../models/UrbitCache.php');
}
if (!class_exists('UrbitCarrier')) {
    require_once(dirname(__FILE__) . '/../models/UrbitCarrier.php');
}
...
abstract class UrbitAbstract extends CarrierModule
{
    ...
    protected function installTabs()
    {
        ...
        if (self::CLASS_PARENT_TAB) {
            ...
            $flag = $this->installModuleTab($this->class_controller_admin, $this->displayName, $id_tab);
        }
        return $flag;
    }
    ...
    {
        return (parent::uninstall() && $this->uninstaller->uninstallTables() && $this->uninstallModuleTab());
    }

    /**
     * Uninstall an Admin Tab (menu)
     * @param string $strTabClass
     * @return boolean
     */
    public function uninstallModuleTab()
    {
        $id_tab = (int)Tab::getIdFromClassName((string)$this->class_controller_admin);
        if ($id_tab != 0) {
            $tab = new Tab($id_tab);
    ...
    public function getConfigurationStatus()
    {
        ...
        if (Db::getInstance()->getValue('SELECT count(id_urbit_rate_service_code) FROM `' . _DB_PREFIX_ . 'urbit_rate_service_code` WHERE `active` = 1') < 1) {
    ...
    {
        $url = $this->context->link->getAdminLink('ShippingOptions') . '&configure=' . $this->name;
        ...
        $ret_zip_code_check = $urbitStoreApi->checkZipCodeDeliverable($user_delivery_address['postcode']);
    ...
    public function hookDisplayOrderConfirmation($params)
    {
        ...
        $order_cart_carrier = Db::getInstance()->executeS('SELECT  id_carrier FROM ' . _DB_PREFIX_ . 'carrier WHERE external_module_name = "urbit" ORDER BY id_carrier DESC LIMIT 1');

        if ($order_cart_carrier[0]["id_carrier"] == $objOrder->id_carrier) {
            $upservice = Db::getInstance()->execute('UPDATE ' . _DB_PREFIX_ . 'urbit_order_cart SET `id_carrier` = ' . $objOrder->id_carrier . ' WHERE id_cart = ' . $objOrder->id_cart);
        }
        ...
        if ($ret_order_cart[0]['del_is_gift']) {
            $delivery_phone = $ret_order_cart[0]['del_receiver_phone'];
        } else {
            $delivery_phone = $ret_order_cart[0]['del_contact_phone'];
        }


        if (!empty($ret_order_cart)) {
            // get product information of the order
            $ret_order_product = UrbitCart::getOrderProducts($objOrder->id_cart);

            $date = DateTime::createFromFormat(
                $DATETIME,
                $ret_order_cart[0]['del_time']
            );
            //format order expected date to ISO standard
            $delivery_expected_at = $date->format('Y-m-d\TH:i:sP');

            $order_data = array(
                'order_retailer_reference_id' => $objOrder->reference,
                'order_delivery_type' => $ret_order_cart[0]['del_type'],
                'order_postal_code' => $ret_order_cart[0]['del_zipcode'],
                'delivery_expected_at' => $delivery_expected_at,
                'order_direction' => "StoreToConsumer",
                'order_total_amount_excl_vat' => $objOrder->total_paid_tax_excl,
                'storelocation_id' => "1",
                'consumer_first_name' => $ret_order_cart[0]['del_fullname'],
                'consumer_last_name' => ". ",
                'consumer_email' => $ret_order_cart[0]['del_contact_mail'],
                'consumer_cell_phone' => $delivery_phone,
                'consumer_consumer_comment' => "good client",
                'address_street' => $ret_order_cart[0]['del_address1'],
                'address_postal_code' => $ret_order_cart[0]['del_zipcode'],
                'address_city' => $ret_order_cart[0]['city'],
                'address_country' => $ret_order_cart[0]['iso_code'],
                'address_care_of' => "SAJ",
                'address_street2' => "",
                'address_company_name' => $ret_order_cart[0]['company'],
                'delivery_delivery_type' => $ret_order_cart[0]['del_type'],
                'delivery_postal_code' => $ret_order_cart[0]['del_zipcode'],
                'delivery_delivery_expected_at' => $delivery_expected_at,
                'pickuplocation_id' => "",
                //'pickuplocation_id' => "3313205a-a51d-4b81-a00c-9c81f1f0c111",
                'order_products' => $ret_order_product,
            );


            // send order information to API to create order
            $ret_create_order = UrbitStoreApi::createOrder($order_data);
    ...
    public function upgrade($version)
    {
        ...
        SET `cl`.`delay` = `arsc`.`delay`';
                    $flag = (Db::getInstance()->query($sql_alter) && Db::getInstance()->query($sql_update) && $this->registerHook('displayRightColumnProduct') && $this->registerHook('actionObjectCarrierUpdateAfter'));
                }
                break;
            case '2.9':
     ...
     * @return \Cart
     */
    private function getCart($id_product, $id_attribute_product, $qty)
    {
        require_once(dirname(__FILE__) . '/models/UrbitCart.php');
